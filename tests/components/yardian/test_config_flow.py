...
async def test_reconfigure_success(hass: HomeAssistant) -> None:
    ...
    with patch(
        "homeassistant.components.yardian.config_flow.AsyncYardianClient.fetch_device_info",
        return_value={"name": "fake_name", "yid": "y1"},
    ):
        result = await hass.config_entries.flow.async_init(
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": "new_host", "access_token": "new_token"},
        )
        assert result2["type"] is FlowResultType.ABORT
        assert result2["reason"] == "reconfigure_successful"
        updated = hass.config_entries.async_get_entry(entry.entry_id)
        assert updated is not None
        assert updated.data["host"] == "new_host"
        assert updated.data["access_token"] == "new_token"


async def test_reconfigure_invalid_auth(hass: HomeAssistant) -> None:
    """Test reconfigure handles invalid auth and shows form error."""
    ...
    with patch(
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": "new_host", "access_token": "bad"},
        )
